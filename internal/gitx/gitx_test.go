...
func TestCommitCycle(t *testing.T) {
	...
	c1, err := r.BuildCommit(ctx, "", "first\n\nOrigoa-Op: create\nOrigoa-Guid: abc\n", []Op{
		{Path: "sp ace/x/.origoa.json", Content: []byte(`{"a":1}`)},
	...
	c2, err := r.BuildCommit(ctx, c1, "second", []Op{
		{Path: "sp ace/x/.origoa.json", Content: []byte(`{"a":2}`)},
	...
	log, err := r.Log(ctx, c2, []string{Literal("sp ace/x/.origoa.json")}, 0)
	if err != nil || len(log) != 2 {
		t.Fatalf("log=%v err=%v", log, err)
	}
	if log[1].Trailers["Origoa-Op"] != "create" || log[1].Body != "" || log[1].Subject != "first" {
