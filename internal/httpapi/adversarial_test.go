...
func TestHTTPAbuse(t *testing.T) {
	...
	for _, p := range []string{
		"/api/artifacts/" + g + "/files/..%2F..%2F.origoa.json", "/api/artifacts/" + g + "/files/%2e%2e", "/api/artifacts/" + g + "/files/.origoa.json",
	...
	a.ok("POST", "/api/repository/folders/move", `{"from":"x","to":".origoa"}`, 400)
