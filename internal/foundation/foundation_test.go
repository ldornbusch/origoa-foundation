...
import (
	...
	"github.com/thomdehoog/origoa/internal/testutil"
)
...
func TestLifecycle(t *testing.T) {
	...
	if l1.Meta.Folder != "" || !strings.HasPrefix(l1.Meta.Path, ".origoa/links/") {
	...
	if err := f.PutAttachment(ctx, r1.Meta.GUID, ".origoa.json", []byte("x")); err == nil {
	...
	if len(log) < 5 || !strings.Contains(log[0].Subject, "moved") || log[len(log)-1].Trailers["Origoa-Op"] != "create" {
...
func TestFormatFidelityAndDirectPush(t *testing.T) {
	...
	raw := "{\r\n\t\"guid\": \"" + g + "\",\r\n\t\"kind\": \"entry\",\r\n\t\"type\": \"requirement\",\r\n\t\"x-tool\": {\"keep\": [1, 2]},\r\n\t\"title\": \"Hand made\",\r\n\t\"fields\": {\r\n\t\t\"priority\": \"low\"\r\n\t}\r\n}"
	head, _ := f.Repo.Head(ctx)
	c, err := f.Repo.BuildCommit(ctx, head, "external push", []gitx.Op{{Path: "ext/" + g + "/.origoa.json", Content: []byte(raw)}})
	if err != nil {
		t.Fatal(err)
	}
