...
func TestHostileInputsAreRejectedCleanly(t *testing.T) {
	...
	cases := []string{
		...
		`{"path":"a/.origoa","type":"requirement","title":"x","fields":{"priority":"low"}}`,
	...
	for _, name := range []string{"../x", "..", "a/b", `a\b`, ".origoa.json", ".origoa", "", strings.Repeat("n", 300), "x\x00y", ":x"} {
...
func TestHostileRepositoryContent(t *testing.T) {
	...
	c, err := f.Repo.BuildCommit(ctx, head, "hostile push", []gitx.Op{
		{Path: "bad/" + g2 + "/.origoa.json", Content: []byte("{not json")},
		{Path: "bad/" + g3 + "/.origoa.json", Content: []byte(`{"guid":"` + ok.Meta.GUID + `","kind":"entry","type":"requirement","title":"GUID thief"}`)}, // claims an existing GUID
		{Path: "bad/" + model.NewGUID() + "/.origoa.json", Content: []byte(`{"guid":"` + model.NewGUID() + `","kind":"link","type":"x","source":"` + ok.Meta.GUID + `","target":"` + ok.Meta.GUID + `"}`)},
		{Path: "bad/.origoa/links/not-a-guid.json", Content: []byte(`{"guid":"` + model.NewGUID() + `","kind":"link","type":"related","source":"` + ok.Meta.GUID + `","target":"` + model.NewGUID() + `"}`)},
		{Path: "bad/.origoa/comments/" + model.NewGUID() + ".json", Content: []byte(`{"guid":"` + model.NewGUID() + `","kind":"comment","subject":"` + model.NewGUID() + `","text":"orphan"}`)},
		{Path: "bad/.origoa/schemas/requirement.json", Content: []byte(`{"type":"other"}`)},
		{Path: "bad/.origoa/workflows/dev.json", Content: []byte(`{"id":"dev","states":[]}`)},
		{Path: ".origoa/scanner.json", Content: []byte(`{"guid_files":["a/b"]}`)},
		{Path: "huge/" + model.NewGUID() + "/.origoa.json", Content: []byte(`{"guid":"` + model.NewGUID() + `","kind":"entry","type":"t","title":"` + strings.Repeat("x", 2<<20) + `"}`)},
		{Path: "bin/" + model.NewGUID() + "/.origoa.json", Content: []byte("\x00\x01\x02\xff\xfe")},
		{Path: "cycle/" + model.NewGUID() + "/.origoa.json", Content: []byte(`{"guid":"11111111-1111-4111-8111-111111111111","kind":"entry","type":"requirement","title":"c1","base":"22222222-2222-4222-8222-222222222222"}`)},
		{Path: "cycle/" + model.NewGUID() + "/.origoa.json", Content: []byte(`{"guid":"22222222-2222-4222-8222-222222222222","kind":"entry","type":"requirement","title":"c2","base":"11111111-1111-4111-8111-111111111111"}`)},
		{Path: "dup/" + model.NewGUID() + "/.origoa.json", Content: []byte(`{"guid":"33333333-3333-4333-8333-333333333333","kind":"entry","type":"requirement","title":"dup a","hid":"REQ-1"}`)},
	})
	if err != nil {
		t.Fatal(err)
	}
