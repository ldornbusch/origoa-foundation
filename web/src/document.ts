    ...
    switch (id) {
      case "props":
        return html`<h4>Document properties</h4>
          ...
          ${(this.schema?.fields ?? []).filter((f) => f.type !== "workflow").map((f) => html`<origoa-field .field=${f} .value=${this.fields[f.id]} .attachments=${v.attachments ?? []}
            @field-change=${(e: CustomEvent) => { const n = { ...this.fields }; if (e.detail.value == null) delete n[e.detail.id]; else n[e.detail.id] = e.detail.value; this.fields = n; }}></origoa-field>`)}
