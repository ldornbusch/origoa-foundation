...
export class Detail extends LitElement {
  ...
  private createVariant() {
    ...
    window.setTimeout(() => {
      const dlg = document.querySelector("origoa-new-dialog") as (HTMLElement & { base: string; baseLabel: string }) | null;
  ...
  private fieldsSection() {
    ...
    return html`<section class="section" id="sec-fields"><h3>Fields</h3>
      ...
      ${fields.map((f) => html`<origoa-field .field=${f} .value=${d.fields[f.id]} .attachments=${this.view?.attachments ?? []}
        ...
        @open-artifact=${(e: CustomEvent) => navigate({ guid: e.detail })}></origoa-field>`)}
      ${extra.map((k) => html`<origoa-field .field=${{ id: k, name: k + " (unschematized)", type: typeof d.fields[k] === "object" ? "json" : typeof d.fields[k] === "boolean" ? "boolean" : "text" }} .value=${d.fields[k]}
        @field-change=${(e: CustomEvent) => { const next = { ...d.fields }; if (e.detail.value == null) delete next[e.detail.id]; else next[e.detail.id] = e.detail.value; this.draft = { ...d, fields: next }; this.touched(); }}></origoa-field>`)}
    </section>`;
  }
  ...
  private historySection() {
    return html`<section class="section" id="sec-history"><h3>History</h3>
      ${this.history.map((h) => html`<div class="history-row"><span class="when" title=${h.time}>${fmtTime(h.time)}</span><span>${h.subject}${h.trailers?.["Origoa-Op"] ? html` <span class="pill">${h.trailers["Origoa-Op"]}</span>` : nothing}</span><span class="mono muted">${h.sha.slice(0, 8)}</span></div>`)}
