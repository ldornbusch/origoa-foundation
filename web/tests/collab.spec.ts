...
test("presence: editing badge appears for the other user and clears on save", async ({ browser }) => {
  ...
  await alice.addInitScript(() => localStorage.setItem("origoa.user", "alice"));
  await bob.addInitScript(() => localStorage.setItem("origoa.user", "bob"));
  ...
  await expect(alice.locator("origoa-detail h2")).toHaveText("bob is typing");
  await c1.close();
  await c2.close();
});
...
test("live updates: creations, transitions and deletions show up in the other session", async ({ browser }) => {
  ...
  await expect(a.locator("origoa-overview .empty-state")).toBeVisible();
  ...
  await expect(a.locator("origoa-detail h2")).toHaveText(`Live ${id}`);
  await b.locator("[data-test=delete]").click();
  await b.locator(".dialog .btn.danger").click();
  await expect(a.locator("origoa-detail .empty-state, origoa-detail .notice.error").first()).toBeVisible();
