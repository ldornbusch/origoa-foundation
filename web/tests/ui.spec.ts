...
// Browser tests of the generic client against a live origoad. They exercise
...
const base = process.env.ORIGOA_URL || "http://127.0.0.1:18090";
const api = base + "/api";
...
test("create an entry through the + button with a schema-generated form", async ({ page }) => {
  ...
  await expect(page.locator("origoa-detail h2")).toHaveText("Boot fast");
  await expect(page.locator("origoa-detail .detail-head .hid")).toContainText("UR");
...
test("edit fields, save with optimistic concurrency, and see history", async ({ page }) => {
  ...
  await expect(page.locator("origoa-detail h2")).toHaveText("Editable");
  ...
  await expect(page.locator("origoa-detail h2")).toHaveText("Editable (v2)");
...
test("relationships: link two artifacts and navigate between them", async ({ page }) => {
  ...
  await page.locator("origoa-picker input").fill(`Linked requirement ${stamp}`);
  ...
  await expect(page.locator("origoa-detail h2")).toHaveText(`Linked requirement ${stamp}`);
...
test("overlays: create a variant and see inherited fields", async ({ page }) => {
  ...
  await page.locator("[data-test=create]").click();
  await expect(page.locator("origoa-detail h2")).toHaveText("Variant A");
...
test("documents: compose from entries and edit in the block editor", async ({ page }) => {
  ...
  await page.locator("origoa-picker input").fill(`Referenced in doc ${stamp}`);
  ...
  await expect(page.locator("origoa-detail h2")).toHaveText(`Referenced in doc ${stamp}`);
});
