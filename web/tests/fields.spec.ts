...
test("all field types round-trip through the generated form", async ({ page }) => {
  ...
  await page.locator("origoa-picker input").fill(target.title);
  ...
  await expect(page.locator("origoa-detail h2")).toHaveText("Everything filled");
  ...
  const f = page.locator("origoa-detail");
  ...
  await expect(page.locator("origoa-detail h2")).toHaveText(target.title);
});
