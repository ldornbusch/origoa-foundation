...
test("script-looking content is rendered as text everywhere", async ({ page }) => {
  ...
  await expect(page.locator("origoa-detail h2")).toHaveText(xss);
...
test("double submit creates exactly one artifact", async ({ page }) => {
  ...
  await expect(page.locator("origoa-detail h2")).toHaveText("only once");
...
test("garbage deep links degrade gracefully", async ({ page }) => {
  ...
  await expect(page.locator("origoa-detail .empty-state")).toContainText("not available");
