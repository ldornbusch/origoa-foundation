# Groundsill — build, test and run.
#
#   make build            build web/dist and bin/groundsilld
...
GROUNDSILL_E2E_DSN ?= postgres://postgres:postgres@127.0.0.1:5432/groundsill_e2e?sslmode=disable
GROUNDSILL_DB ?= postgres://postgres:postgres@127.0.0.1:5432/groundsill?sslmode=disable
...
define with_server
	rm -rf /tmp/groundsill-$(1).git; \
	./bin/groundsilld -repo /tmp/groundsill-$(1).git -addr 127.0.0.1:$(2) -web web/dist -db "$(GROUNDSILL_E2E_DSN)" > /tmp/groundsill-$(1).log 2>&1 & pid=$$!; \
	for i in $$(seq 1 50); do curl -sf http://127.0.0.1:$(2)/api/repository >/dev/null && break; sleep 0.2; done; \
	$(3); status=$$?; \
	kill $$pid 2>/dev/null; rm -rf /tmp/groundsill-$(1).git; exit $$status
endef
